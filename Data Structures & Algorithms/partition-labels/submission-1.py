    ...
    def partitionLabels(self, s: str) -> List[int]:
        ...
        for char in s:
            ...
            if covered and len(substr_chars) > 0:
                ...
                print(f"transitioning into new substring")
                substr_chars = set(char)
                freq_map[char] -= 1
                length = 1
                print(f"post transition, freq_map = {freq_map} and substr_chars = {substr_chars}\n\n")
            else:
                print(f"reducing freq_map[{char}], adding it to substr chars")
                freq_map[char] -= 1
                substr_chars.add(char)
                length += 1
                print(f"freq_map now {freq_map}")
                print(f"substr chars now {substr_chars}")
                print(f"\n\n")
        lengths.append(length)
